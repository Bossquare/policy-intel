...
| `export.html?id=` | 导出预览：按 210 × 297 mm 纸张排版的纯净文档，可打印或另存为 PDF |
...
简报相关页面（归档列表、简报详情）右下角常驻一个「下载简报」浮动按钮，点开即可逐期选择
**PDF** 或 **HTML**；归档页卡片上的「↓ 下载」会直接展开坞并高亮该期。入口用 `assets/css/style.css`
的 `.dl-dock` 定义，由 `assets/js/export.js` 的 `mountDock()` 挂载（页面 `data-page` 为
`briefs` / `brief` 时生效），样式与页面上原有的下载按钮无关，两处文案都不出现纸张规格字样。

- **PDF** — 有预渲染的静态 PDF（`assets/briefs/<期号>.pdf`）时一键直下；
  未预渲染时自动打开导出预览并唤起打印对话框，选「另存为 PDF」即可。
- **HTML** — 下载**自包含**单文件（内联样式与徽标，无任何外部依赖），双击即可离线打开。

两类文件都按 **210 × 297 mm** 纸张排版，正文宋体、标题黑体，含文档头、核心结论、
...
│   ├── js/export.js             简报导出：版式生成、PDF / HTML 下载、右下角浮动下载坞
...
```bash
git add -A
git commit -m "chore: 更新 2026-W38 情报数据"
git push
```
